from typecheck.typecheck import parse_inputs, Input, TypeCheckError
...
class test_parse_inputs_happy_path(unittest.TestCase):
    ...
    def test_mult_kwonlyargs(self):
        def fnc(*, x, y, z): pass
        expect = Input(
            kwonlyargs={"x": 1, "y": 2, "z": 3}
        )
        actual = parse_inputs(fnc, kwargs={"x": 1, "y": 2, "z": 3})
        self.assertEqual(expect, actual)
    ...
    def test_args_varargs_kwonlyargs_and_varkw(self):
        def fnc(a, b, *args, c, d, **kwargs): pass
        expect = Input(
            args={"a": 1, "b": 2},
            varargs=[3, 4],
            kwonlyargs={"c": 3, "d": 4},
